    ...
    return(
        <div className="p-3 grid grid-cols-(--my-grid-calls) gap-4 items-center justify-between bg-white">
            ...
            <div className="flex gap-4 order-1 md:order-2">
                <img src={imageUrl} alt={title} className="md:w-36 w-[70%] md:h-36 h-62.5 object-cover object-top p-1 border border-gray-200 rounded-sm"/>
                <div className="flex flex-col gap-1 justify-center items-center w-20% md:w-auto">
                    ...
                    <h1 className="text-6xl font-semibold border-b border-b-gray-500">{day}</h1>
                    <h2 className="text-xl">{month.toUpperCase()}</h2>
                    <p className="text-lg md:text-base">{year}</p>
                </div>
            </div>
        </div>
    )
